...
class ServerSyncService {
  static Future<List<String>> _uploadEvidences(List<EvidenceFile> evidences, String storagePathPrefix) async {
    final urls = <String>[];
    for (final e in evidences) {
      final file = File(e.path);
      if (!await file.exists()) continue;
      final ref = FirebaseStorage.instance.ref('$storagePathPrefix/${e.name}');
  ...
  static Future<String?> uploadInspection(Inspection item) async {
    ...
    try {
      final evidenceUrls = await _uploadEvidences(item.evidences, 'inspections/${user.uid}/${item.id}');
  ...
  static Future<String?> uploadPakhshRecord(PakhshRecord item) async {
    ...
    if (user == null) return 'ابتدا وارد حساب سرور شوید.';
    try {
      final evidenceUrls = await _uploadEvidences(item.evidences, 'pakhsh_records/${user.uid}/${item.id}');
